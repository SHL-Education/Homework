/* RB 트리는 AVL 트리 보다 검색 속도는 느리다.
   그러나 빈번한 생성과 삭제가 발생할 경우
   AVL 트리는 빈번한 회전 때문에 삽입과 삭제의 속도가 느려진다.
   반면 RB 트리는 AVL 에 비해 회전의 횟수가 적기 때문에
   빈번하게 발생하는 삽입과 삭제에 대응할 수 있고
   검색 속도도 나름 준수한 편이라 구글, MS, facebook 등등의
   기업들에서 각광 받고 있는 매우 뛰어난 알고리즘중 하나다. */
