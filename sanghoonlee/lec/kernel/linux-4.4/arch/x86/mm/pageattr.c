			     ...
			     unsigned int *level)
{
	...
	*level = PG_LEVEL_NONE;
	...
	// pgd == swapper_pg_dir + 페이지 테이블의 인덱스
	// pgd 는 4 단계 페이징에서 첫 번째 테이블임
	if (pgd_none(*pgd))
		return NULL;

	// address == 시스템 콜 테이블 주소
	// pud == 0xffff 8800 0000 0000 혹은 0xffff c800 0000 0000 + 테이블 인덱스
	// pud 는 4 단계 페이징에서 두 번째 테이블임
	...
	*level = PG_LEVEL_2M;
	...
	// *level = 1 로 지정하면서 페이지 크기 4 KB 를 지정
	*level = PG_LEVEL_4K;
...
{
	/* lookup_address_in_pgd((swapper_pg_dir + 페이지 테이블의 인덱스),
							시스템 콜 테이블 주소, level) */
...
static int __change_page_attr(struct cpa_data *cpa, int primary)
{
	...
	if (cpa->flags & CPA_PAGES_ARRAY) {
		...
		address = *cpa->vaddr;
repeat:
	/* 4 단계 페이징을 거쳐 실제 물리 메모리 획득
	   주의할 것은 이미 시스템 콜 테이블은 부팅시 초기화 되므로
	   새로운 페이지를 할당한 것이 아니라
	   이미 존재하는 페이지를 통해 물리 주소를 획득한 것임
	   그러므로 kpte 는 명확한 값을 가지고 있음 */
				    ...
				    struct page **pages)
{
	...
	*/
	...
	시스템 콜 테이블 주소 시작 위치부터 3 개의 페이지에 읽고 쓰기 권한을 줌 */
...
{
	// change_page_attr_set(&addr, numpages, __pgprot(2), 0)
