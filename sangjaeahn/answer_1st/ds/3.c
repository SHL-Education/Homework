...
void init_arr(int *arr)	// int형 100개 배열에 1~131072 수를 셋팅함.
{
	int i;
...
mem_man *get_mm_node(void)	// mem_man 구조체 타입의 노드를 생성함.
{
	mem_man *tmp;
...
void mem_man_info(mem_man *mm[PAGENUM])
{
	...
	for (i = 0; i < PAGENUM; i++)
	{
		...
		while (tmp)
		{
			...
			printf("idx = %2d, used_flag = %s, usage = %6d, link = %s\n",
					tmp->idx, tmp->used_flag ? "Yes" : "No", tmp->usage, tmp->link ? "Yes" : "No");
			tmp = tmp->link;
		}
	}
